...
static void show_call_state(pjsua_call_id call_id, char *number, pjsip_inv_state inv_state, int error_code)
{
    ...
    case PJSIP_INV_STATE_NULL:
    	PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe   show_call_state inv_state PJSIP_INV_STATE_NULL"));
    ...
    case PJSIP_INV_STATE_INCOMING:
    	PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe   show_call_state inv_state PJSIP_INV_STATE_INCOMING"));
/*        if (current_call != call_id)
        {
        	PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe   show_call_state inv_state PJSIP_INV_STATE_INCOMING111"));
            return;
        }
        else if (ptt_get_state() == PTT_STATUS_BUSY)
        {
        	PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe   show_call_state inv_state PJSIP_INV_STATE_INCOMING2222"));
            if(group_number_compare(number))
                ptt_close_media();
            else
                return;
        }
        PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe   show_call_state inv_state PJSIP_INV_STATE_INCOMING3333"));
    ...
    case PJSIP_INV_STATE_DISCONNECTED:
    {
        char tmp[64] = {0};
        if(g_current_group)
            PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe DISCONNECTED current_call %d call_id %d pttstate %d current_grp %.*s dest %s", current_call, call_id, ptt_get_state(), g_current_group->number.slen, g_current_group->number.ptr, dest_num));
        else
            PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe DISCONNECTED current_call %d call_id %d pttstate %d dest %s", current_call, call_id, ptt_get_state(), dest_num));
    ...
    showCallStateId = (*jni_env)->GetMethodID(jni_env, jni_callback_class, "showCallState", "(Lcom/zzy/ptt/model/CallState;)V");
			     ...
			     pjsip_rx_data *rdata)
{
    ...
    if(g_current_group)
        PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe on_incoming_call call_id %d, current_call %d, pttstate %d dest %s g_current_group %.*s in_num %s", call_id, current_call, ptt_get_state(), dest_num, g_current_group->number.slen, g_current_group->number.ptr, in_number));
    else
        PJ_LOG(3, (THIS_FILE, ">>>>>>sxsexe on_incoming_call call_id %d, current_call %d, pttstate %d dest %s in_num %s", call_id, current_call, ptt_get_state(), dest_num, in_number));
        
    PJ_LOG(4,(THIS_FILE, "sxsexe on_incoming_call Call %d  current_call=%d..", call_id, current_call));
                      ...
                      pjsip_rx_data *rdata, pjsua_acc_id acc_id)
{
    ...
    PJ_LOG(3,(THIS_FILE,"sxsexe  MESSAGE from %.*s: %.*s (%.*s)",
	      (int)from->slen, from->ptr,
...
pj_status_t app_main(void)
{
...
#if (PJSIP_EXTEND_ZED || PJSIP_EXTEND_ZZY)
    ...
    if (!jni_callback_class)
    {
        jni_callback_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/zzy/ptt/jni/JniCallback"));
    ...
    if (!call_state_class)
    {
        call_state_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/zzy/ptt/model/CallState"));
    ...
    if (!group_info_class)
    {
        group_info_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/zzy/ptt/model/GroupInfo"));
    ...
    {
        member_info_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/zzy/ptt/model/MemberInfo"));
    ...
    {
        ptt_group_status_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/zzy/ptt/model/PttGroupStatus"));
